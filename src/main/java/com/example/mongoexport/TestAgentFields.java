...
/**
 * Test to analyze UltraAgentPerformanceExporter fields
 */
public class TestAgentFields {
    private static final Logger logger = LoggerFactory.getLogger(TestAgentFields.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            logger.info("=== Analyzing Agent Collection Fields ===");
            ...
            int docCount = 0;
            for (Document agent : agents.find().limit(5000)) {
                docCount++;
                analyzeDocument(agent, "", fieldTypes, fieldValues, nullCounts, totalCounts);
            }
            
            logger.info("Analyzed {} agent documents", docCount);
            
            // Report foreign key fields
            logger.info("\n=== Foreign Key Fields (ObjectId references) ===");
            for (Map.Entry<String, Set<String>> entry : fieldTypes.entrySet()) {
                if (entry.getValue().contains("ObjectId") && !entry.getKey().equals("_id")) {
            ...
            // Check realmData structure
            logger.info("\n=== Checking realmData Structure ===");
            for (Document agent : agents.find().limit(3)) {
                logger.info("\nAgent: {}", agent.getString("fullName"));
                Document realmData = (Document) agent.get("realmData");
                if (realmData != null) {
                    logger.info("  realmData keys: {}", realmData.keySet());
                    
                    // Check brokerages structure
                    Object brokeragesObj = realmData.get("brokerages");
                    if (brokeragesObj != null) {
                        logger.info("  brokerages type: {}", brokeragesObj.getClass().getSimpleName());
                        if (brokeragesObj instanceof List && !((List<?>) brokeragesObj).isEmpty()) {
                            Object firstBrokerage = ((List<?>) brokeragesObj).get(0);
                            logger.info("  first brokerage type: {}", firstBrokerage.getClass().getSimpleName());
                            if (firstBrokerage instanceof Document) {
                                logger.info("  first brokerage keys: {}", ((Document) firstBrokerage).keySet());
                            }
                        }
                    }
                }
    ...
    private static void analyzeDocument(Document doc, String prefix, 
        ...
        for (String key : doc.keySet()) {
            ...
            // Skip deep nesting
            if (fullKey.split("\\.").length > 2) continue;
            ...
            if (value instanceof Document && !key.equals("realmData")) {
