...
public class ConfigurationBasedExporter extends AbstractUltraExporter {
    ...
    public ConfigurationBasedExporter(DiscoveryConfiguration config) {
        ...
        logger.info("Total fields: {}, Included fields: {}", 
            config.getFields().size(), includedFields.size());
    }
    ...
    private static ExportOptions buildExportOptions(DiscoveryConfiguration config) {
        return ExportOptions.builder()
            ...
            .enableRelationExpansion(!config.getRequiredCollections().isEmpty())
            .expansionDepth(config.getDiscoveryParameters().getExpansionDepth())
            .build();
    }
    ...
    protected String[] buildComprehensiveHeaders() {
        return includedFields.stream()
            .map(field -> configuration.getExportSettings().isUseBusinessNames() ?
                field.getBusinessName() : field.getFieldPath())
            .toArray(String[]::new);
    }
    ...
    private void cacheRequiredCollections() {
        ...
        logger.info("Cached {} collections with {} total documents",
    ...
    private void writeHeader(CSVWriter csvWriter) {
        String[] header = includedFields.stream()
            .map(field -> configuration.getExportSettings().isUseBusinessNames() ?
                field.getBusinessName() : field.getFieldPath())
            .toArray(String[]::new);
    ...
    private void processBatch(List<Document> batch, CSVWriter csvWriter) {
        for (Document doc : batch) {
            ...
            if (relationExpander != null) {
                doc = relationExpander.expandDocument(doc, configuration.getCollection(), 
                    configuration.getDiscoveryParameters().getExpansionDepth());
            }
    ...
    private void logProgress(int processed, long total, long startTime) {
        ...
        logger.info("Progress: {}/{} documents ({:.1f}%), Rate: {:.0f} docs/sec, ETA: {} seconds",
            processed, total, (processed * 100.0 / total), rate, remaining);
    }
